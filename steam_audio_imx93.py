#!/usr/bin/env python3
# Audio streaming script for i.MX93 MCM SoM by Compulabs (Yocto/Distro Linux)
# Optimized for embedded Linux systems with ALSA audio support
# Dependencies (install with pip or uv):
#   livekit, livekit-api, sounddevice, python-dotenv, numpy, asyncio
...
# Audio configuration for i.MX93
SAMPLE_RATE = 48000  # 48kHz - standard for embedded audio
NUM_CHANNELS = 1
FRAME_SAMPLES = 480  # 10ms at 48kHz - required for APM
BLOCKSIZE = 2048  # Optimized for embedded systems (lower latency)
...
class AudioStreamer:
    ...
    def start_audio_devices(self):
        """Initialize and start audio input/output devices for i.MX93 MCM SoM"""
        try:
            self.logger.info("Starting audio devices for i.MX93 MCM SoM...")
            
            # List all devices for debugging
            list_audio_devices()
            
            # Dynamic device selection for embedded Linux (ALSA)
            # Prefer USB audio devices, then i.MX93-specific audio devices
            input_device = None
            output_device = None
            
            # Allow override via environment variables
            input_device_env = os.environ.get("AUDIO_INPUT_DEVICE")
            output_device_env = os.environ.get("AUDIO_OUTPUT_DEVICE")
            
            if input_device_env:
                try:
                    input_device = int(input_device_env)
                    self.logger.info(f"Using input device from environment: {input_device}")
                except ValueError:
                    self.logger.warning(f"Invalid AUDIO_INPUT_DEVICE: {input_device_env}")
            
            if output_device_env:
                try:
                    output_device = int(output_device_env)
                    self.logger.info(f"Using output device from environment: {output_device}")
                except ValueError:
                    self.logger.warning(f"Invalid AUDIO_OUTPUT_DEVICE: {output_device_env}")
            
            # Auto-detect devices if not set via environment
            devices = sd.query_devices()
            
            if input_device is None:
                # Prefer USB audio, then look for i.MX93 audio chips (sgtl5000, wm8960, etc.)
                    ...
                    if isinstance(device, dict):
                        ...
                        if device['max_input_channels'] >= NUM_CHANNELS:
                            ...
                            if 'usb' in name_lower:
                                ...
                                self.logger.info(f"Found USB input device: {device['name']}")
                                break
                            # Look for i.MX93 audio codecs
                            elif any(codec in name_lower for codec in ['sgtl5000', 'wm8960', 'imx', 'alsa', 'hw']):
                                input_device = i
                                self.input_device_name = device['name']
                                self.logger.info(f"Found i.MX93 audio input device: {device['name']}")
                                break
                ...
                if input_device is None:
                    ...
                    if default_in is not None:
                        ...
                        self.input_device_name = device_info.get('name', 'Default Input')
                        self.logger.info(f"Using default input device: {self.input_device_name}")
            
            if output_device is None:
                # Prefer USB audio, then look for i.MX93 audio chips
                for i, device in enumerate(devices):
                    if isinstance(device, dict):
                        name_lower = device['name'].lower()
                        if device['max_output_channels'] >= NUM_CHANNELS:
                            # Prefer USB audio devices
                            if 'usb' in name_lower:
                                output_device = i
                                self.logger.info(f"Found USB output device: {device['name']}")
                                break
                            # Look for i.MX93 audio codecs
                            elif any(codec in name_lower for codec in ['sgtl5000', 'wm8960', 'imx', 'alsa', 'hw']):
                                output_device = i
                                self.logger.info(f"Found i.MX93 audio output device: {device['name']}")
                                break
                ...
                if output_device is None:
                    ...
                    if default_out is not None:
                        ...
                        self.logger.info(f"Using default output device: {sd.query_devices(default_out).get('name', 'Default Output')}")
            ...
            if input_device is not None:
                ...
                if isinstance(device_info, dict):
                    ...
                    if default_rate != SAMPLE_RATE:
                        self.logger.info(f"Device default sample rate is {default_rate}, using {SAMPLE_RATE}")
            
            if output_device is not None:
                device_info = sd.query_devices(output_device)
                if isinstance(device_info, dict):
                    self.logger.info(f"Output device info: {device_info}")
            
            self.logger.info(f"Using input device: {input_device} ({self.input_device_name}), output device: {output_device}")
            ...
            if not self.input_stream.active:
                raise RuntimeError("Input stream failed to start")
            if not self.output_stream.active:
                raise RuntimeError("Output stream failed to start")
    ...
    def start_keyboard_handler(self):
        """Start keyboard input handler in a separate thread (Linux only)"""
        ...
        with self.output_lock:
            ...
            if len(self.output_buffer) < bytes_needed:
                ...
                if available_bytes > 0:
                    ...
                    outdata.fill(0)
                self.logger.debug(f"Output buffer underflow: needed {bytes_needed}, had {buffer_size_before}")
            else:
                ...
                self.logger.debug(f"Played {bytes_needed} bytes from buffer, remaining: {len(self.output_buffer)}")
...
async def main(participant_name: str, enable_aec: bool = True):
    ...
    logger.info("=== STARTING AUDIO STREAMER FOR i.MX93 MCM SoM ===")
    ...
    async def receive_audio_frames(stream: rtc.AudioStream, participant: rtc.RemoteParticipant):
        ...
        async for frame_event in stream:
            ...
            if frames_received <= 5:
                logger.info(f"Received audio frame {frames_received} from {participant_name}, data length: {len(frame_event.frame.data)}")
            elif frames_received % 100 == 0:
                logger.info(f"Received {frames_received} frames total from {participant_name}, latest data length: {len(frame_event.frame.data)}")
            ...
            if streamer.active_remote_participant_id == participant_id and streamer.remote_playback_enabled:
                ...
                with streamer.output_lock:
                    buffer_size_before = len(streamer.output_buffer)
                    streamer.output_buffer.extend(audio_data)
                    logger.debug(f"Added {len(audio_data)} bytes to output buffer from {participant_name}, new size: {len(streamer.output_buffer)} (was {buffer_size_before})")
    ...
    def on_track_subscribed(
        ...
        logger.info(f"Track subscribed: {publication.sid} from participant {participant.sid} ({participant.identity}), kind: {track.kind}")
        if track.kind == rtc.TrackKind.KIND_AUDIO:
            ...
            if streamer.active_remote_participant_id is None:
                ...
                logger.info(f"Activating remote playback for first subscribed participant: {participant.identity} ({participant.sid})")
                ...
                logger.info(
                    f"Ignoring additional remote audio track from {participant.identity} ({participant.sid}); active playback is participant {streamer.active_remote_participant_id}"
                )
    ...
    def on_track_published(
        ...
        logger.info(
            f"Track published: {publication.sid} from participant {participant.sid} ({participant.identity})"
        )
    ...
    def on_participant_connected(participant: rtc.RemoteParticipant):
        ...
        logger.info(f"Added participant to tracking: {participant.identity} ({participant.sid})")
        logger.info(f"Current participants: {[p.identity for p in room.remote_participants.values()]}")

    @room.on("participant_disconnected")
    def on_participant_disconnected(participant: rtc.RemoteParticipant):
        logger.info(f"Participant disconnected: {participant.sid} ({participant.identity})")
        # Remove participant from our tracking
        with streamer.participants_lock:
            if participant.sid in streamer.participants:
                del streamer.participants[participant.sid]
                logger.info(f"Removed participant from tracking: {participant.identity} ({participant.sid})")
        ...
        if streamer.active_remote_participant_id == participant.sid:
            logger.info(f"Active remote participant disconnected: {participant.identity} ({participant.sid}); releasing playback and clearing buffer")
    ...
    def on_connected():
        ...
        logger.info(f"Local participant: {room.local_participant.identity} ({room.local_participant.sid})")
        logger.info(f"Current remote participants: {len(room.remote_participants)}")
        for sid, participant in room.remote_participants.items():
            logger.info(f" - {participant.identity} ({sid})")
    ...
    try:
        ...
        # Start keyboard handler (may not work on headless systems, but won't crash)
        try:
            logger.info("Starting keyboard handler...")
            streamer.start_keyboard_handler()
        except Exception as e:
            logger.warning(f"Could not start keyboard handler (may not have TTY): {e}")
        ...
        # Log current participants after connection
        logger.info(f"Local participant: {room.local_participant.identity} ({room.local_participant.sid})")
        logger.info(f"Current remote participants: {len(room.remote_participants)}")
        for sid, participant in room.remote_participants.items():
            logger.info(f" - {participant.identity} ({sid})")
            for track_pub in participant.track_publications.values():
                logger.info(f"   - Track: {track_pub.sid}, kind: {track_pub.track.kind if track_pub.track else 'unknown'}")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="LiveKit bidirectional audio streaming with AEC for i.MX93 MCM SoM")
    parser.add_argument(
        "--name", 
        "-n",
        type=str,
        default="imx93-audio-streamer",
        help="Participant name to use when connecting to the room (default: imx93-audio-streamer)"
    )
    parser.add_argument(
        "--disable-aec",
        action="store_true",
    ...
    # Set up logging - use /var/log if writable, otherwise current directory
    log_file = "steam_audio_imx93.log"
    try:
        # Try to write to /var/log if possible (may require permissions)
        if os.access("/var/log", os.W_OK):
            log_file = "/var/log/steam_audio_imx93.log"
    except:
        pass
